...
Goサーバーを停止して、第3項の `go run ./cmd/api` で再起動します。起動ログに `using mock guide generator for development` が表示されます。DBには `20260908000100_support_multiple_guides.sql` までのmigrationを適用しておきます。
...
5. 2件目を開いてタイトルや説明を編集し、保存完了後に「レビュー完了」を押します。全件が確定し、利用者のガイド一覧へ2件とも表示されることを確認します。別の支援では2件目を開かずに確定できることも確認できます。
