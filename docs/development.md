...
8. CREATE後の画像アップロード・AI生成・下書き編集では映像を停止し、音声は続くことを確認します。作成中の再接続も音声だけを接続します。「レビュー完了」で全ガイドの保存が成功すると支援がENDEDになり、両端の音声と共有が終了することを確認します。保存失敗・応答不明の間は音声を続け、全件保存の確定後だけ終了することを確認します。
...
ガイド作成中の音声継続と映像停止、全件保存成功時の通話終了、最大化状態の復元、外側クリックと直接復帰、キー同時押し・ドラッグ表示は2台のWindows実機で別途確認します。旧版のGUIDE_SAVEDセッションに限り、通話・共有の復元と手動終了を互換動作として確認します。これらの実機確認を含むMVPの完成条件は[仕様第17章・第18章](specification.md#17-受け入れテスト)に従います。
