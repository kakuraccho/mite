...
> 最終更新: 2026-09-03  
> 関連文書: [Mite プロダクト仕様書](./Mite_プロダクト仕様書.md)  
...
現時点では「ガイド作成 → ガイド利用」の正常系、保存情報、対象OSなどに未決定事項が残る。そのため、特定のフレームワークやクラウド製品を固定せず、変更しやすい論理構成と契約を先に定義する。
...
| **設計案** | v0.1で推奨する構成。プロトタイプ・技術調査の結果で変更できる。 |
| **TBD** | 決定前。実装開始またはリリースまでに判断が必要。 |
| **MVP外** | 現在のMVPには実装しない。 |

競合時は、プロダクト仕様書の確定事項を本書より優先する。本書内では詳細章を正本とし、冒頭の要約は詳細章への索引として扱う。

本書で追加する安全要件は、v0.1の推奨判断である。プロダクト仕様の未決定事項に影響するものは、実装前にプロダクト仕様の意思決定ログへ反映し、チームで合意する。本書だけでUX上の判断を確定したことにはしない。
...
- 複数端末で共有する業務状態はバックエンドを正本とする。
- 同意した画面・音声・定期取得を取り消すローカル停止は、バックエンドへ到達できなくても即時に有効にする。
- バックエンドは、MVPでは機能境界を持つ単一アプリケーションを第一候補とする。
...
| コンポーネント | 主な責務 | 区分 |
|---|---|---|
| 利用者クライアント | 支援入口、画面取得、着信、音声、画面共有、マーキング表示、ガイド実行。共有同意・ローカル停止はv0.1安全要件 | プロダクト制約 / 安全要件 / 詳細TBD |
| 家族クライアント | 依頼確認、発信、共有画面閲覧、マーキング送信。オンライン表示、ガイド編集・保存の詳細は未決定 | プロダクト制約 / 設計案 / 詳細TBD |
| 制御系バックエンド | 認証・認可、家族関係、依頼・セッション・ガイド状態、状態同期、ファイル参照権限、監査 | 設計案 |
...
- リアルタイム通信アダプターは、接続・切断・品質などの**観測結果**を制御系バックエンドへ報告する。
- 業務状態の遷移、依頼claim、参加権限の発行・失効、セッション終了の正本は制御系バックエンドだけが変更する。
...
sequenceDiagram
    ...
    alt ガイドを作らない
        ...
        B-->>F: 下書き・レビュー（詳細TBD）
        F->>B: 保存・利用可能化、支援セッション終了
    end
    B-->>U: 権限失効・終了状態
    B-->>F: 権限失効・終了状態
```

図は現在のプロダクト正常系を表す。解決を誰がどの操作で確定するか、ガイド作成中も支援セッションを維持するか、先に支援を終了して非同期で作成するかはTBDである。技術上は、依頼の解決、セッション終了、ガイド作成ジョブを別の状態として扱い、どのUXにも対応できるようにする。
...
| `PENDING` | バックエンド | 家族が未確認 | 明示的な確認、取消、期限切れ |
| `ACKNOWLEDGED` | バックエンド | 家族が確認済みだが未解決 | 支援成功、取消、期限切れ |
| `RESOLVED` | バックエンド | 問題解決済み | 終端 |
| `CANCELLED` | バックエンド | 利用者が取消済み | 終端 |
| `EXPIRED` | バックエンド | 有効期限切れ | 終端 |
...
| ガイド | `guide_generation.queued`, `guide_generation.succeeded`, `guide_generation.failed`, `guide.version_saved`, `guide.archived`, `guide.deleted` |
...
着信応答と共有同意をUI上で一操作にまとめるかはTBDだが、論理契約では別の結果として記録する。接続成立前にバックエンドへ到達できない場合も、各クライアントはメディアをローカル停止し、資格情報はconnection leaseで失効する。
...
方式候補の比較は技術検証で行う。v0.1ではWebRTC、WebSocket等を採用決定しない。
...
- 左端バー案を採用する場合も4pxバーを唯一の入口にせず、見つけやすいボタン、ショートカット、タスクトレイ等から少なくとも1つの代替入口を提供する。具体方式はUX検証で決める。
...
この流れは安全側の設計案である。素材候補を誰が・どの端末で選ぶか、利用者と家族のどちらが機微情報を確認するか、レビュー項目と画面数はプロトタイプ後に確定する。システムが候補やマスクを提案しても、選択・切り抜き・マスク結果を人が確認するまではガイド版を利用可能にしない。
...
問題解決後もレビューのため論理セッションを残す場合、ライブ画面・定期取得は既定で停止する。音声等を続ける必要がある場合は、継続する機能と目的を表示し、必要な同意を改めて得る。
...
- MVP候補は、1ステップずつ表示し、利用者が「次へ」を押す方式とする。
- ガイドプレイヤーは「戻る」「中断」「最初から」「家族に聞く」を扱える構造にする。実際に表示する操作はUX検証で決める。
- ガイドと操作対象の画面を行き来せずに済む小型・最前面表示を検証する。
...
「5秒ごとの取得」はプロダクト仕様上も検証候補であり、v0.1の本番既定値にはしない。
...
| ガイド下書きの作成・編集 | 参加範囲はTBD | 許可された家族だけ可とする初期案 | 生成処理は提案だけ。保存判断不可 |
| ガイド版の保存 | 確認参加の要否はTBD | 許可された家族が最終判断 | 不可 |
...
- 状態不一致を検知したら、バックエンドのスナップショットを取得して収束させる。
...
- バックエンド障害時も、利用者はローカルで画面共有を止められる。
- 通知障害で支援依頼を失わない。
- 一時切断後は正本から状態を復元できる。
- 保存済みガイドを途中状態で見せない。
...
- `CallAttempt.CONNECTING`中、メディア接続後かつ`SupportSession`作成前にバックエンド障害・中止が起きても、ローカル停止とconnection leaseにより送受信が止まること
...
最初の操作プロトタイプでは次をFakeにする。

- 画面取得: 用意したダミー画像を返す
- 音声・画面共有: 接続状態だけを再現する
- 通知: 家族画面へ擬似通知を表示する
- バックエンド: メモリ内の状態機械で代替可能
- ガイド生成: 固定テンプレートまたは手動入力を返す
- 障害: 切断、通知失敗、保存失敗、状態ズレを操作で発生させる

実データ、実アカウント、外部AIを使わず、正常系と状態表示を先に検証する。
...
| ADR-004 | バックエンド構成・配置 | 開発速度、運用、データ地域、費用 | TBD |
...
1. **ガイド作成 → ガイド利用の正常系を決める**: 作成開始、下書き、レビュー、保存、一覧、1ステップ表示、完了、家族支援への切替を3レーンで通す。
2. **Fakeプロトタイプで正常系を検証する**: 実通信・実画面・外部AIを使わず、利用者の迷いと家族の負担を観察して仮決定を更新する。
3. **分岐・異常系・状態ズレを決める**: 自力解決、応答なし、部分障害、双方の表示不一致、生成・保存失敗などを表へ追加し、復旧・終端を決める。
4. **保存情報を決める**: 各データの目的、取得契機、閲覧・編集者、保存場所、保持期間、削除、外部送信を決める。
5. **技術調査を行う**: TS-001〜TS-009を必要範囲で実施し、実現性、性能、安全性、費用を比較する。
6. **技術設計を確定する**: 状態、API、権限、データ、通信、運用をADRへ反映し、実装可能な版として本書を更新する。
...
8. ガイド作成中に支援セッションを維持するか
9. ガイド下書きの入力、編集項目、レビュー参加者
...
根拠資料の表記は、[Mite プロダクト仕様書](./Mite_プロダクト仕様書.md) §0.1を引き継ぐ。
...
| 支援中の定期取得、5秒間隔案 | [SF] | §9.3で機能フラグOFFを既定とし、採否・頻度を技術検証対象に設定 |
| 家族が保存要否を判断し、Miteが下書きを用意する体験 | [PS]、[SF] | §4.5、§8。生成方式は差し替え可能、最終保存は人が判断 |
| 保存済みガイドの一覧、1ステップ表示、家族支援への切替 | [PS]、[CV] | §8.4、`GuideRun`、支援依頼の文脈項目へ反映 |
