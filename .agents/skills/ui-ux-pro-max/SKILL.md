...
Installation note: the upstream Claude plugin paths are adapted for this project's `.agents/skills` installation. Run the examples below from the repository root. From another working directory, resolve `scripts/search.py` relative to this SKILL.md and invoke its absolute path.
