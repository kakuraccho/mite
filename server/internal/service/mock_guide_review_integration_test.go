...
func TestMockGuideReviewPostgresIntegration(t *testing.T) {
	...
	batch, err := guides.CreateGuideMaterialBatch(ctx, CreateGuideMaterialBatchCommand{Meta: meta("mock-batch"), SupportSessionID: guideIntegrationSession, ExpectedSessionRevision: 3, CaptureIntervalSeconds: 5, ExpectedItemCount: 1, CapturedFrom: &captured, CapturedTo: &captured})
	if err != nil {
		t.Fatal(err)
	}
