...
func TestCancelMultipleGuidesPostgresIntegration(t *testing.T) {
	...
	batch, err := guides.CreateGuideMaterialBatch(ctx, CreateGuideMaterialBatchCommand{Meta: meta("cancel-batch"), SupportSessionID: guideIntegrationSession, ExpectedSessionRevision: 3, CaptureIntervalSeconds: 5, ExpectedItemCount: 1, CapturedFrom: &captured, CapturedTo: &captured})
	if err != nil {
		t.Fatal(err)
	}
